...
import { 
  ...
  useWindowDimensions, LayoutAnimation, UIManager, RefreshControl 
} from 'react-native';
import { useRouter, Stack } from 'expo-router';
import { 
  Github, Linkedin, Send, ExternalLink, Download, User, 
  Database, Shield, Layout, Server, Lock, Mail,
  Code, Coffee, Terminal, ChevronRight, Briefcase, ArrowUp
...
const getTechIcon = (tag: string, color: string, size: number = 14) => {
  ...
  if (t.includes('react') || t.includes('front') || t.includes('next') || t.includes('native')) return <Layout size={size} color={color} />;
  if (t.includes('java') || t.includes('spring')) return <Coffee size={size} color={color} />;
  if (t.includes('data') || t.includes('sql') || t.includes('firebase') || t.includes('postgres') || t.includes('supabase')) return <Database size={size} color={color} />;
  if (t.includes('security') || t.includes('auth') || t.includes('oauth') || t.includes('jwt')) return <Shield size={size} color={color} />;
  if (t.includes('node') || t.includes('express') || t.includes('api')) return <Server size={size} color={color} />;
  if (t.includes('ts') || t.includes('type')) return <Code size={size} color={color} />;
...
export default function PortfolioHome() {
  ...
  const fetchData = useCallback(async (isRefresh = false) => {
    ...
    try {
      ...
      setError('Failed to load portfolio data. Using offline version.');
      setProjects(FALLBACK_PROJECTS);
    } finally {
      setLoading(false);
      setRefreshing(false);
  ...
  const renderHeader = () => (
    <View style={styles.header}>
      <View style={styles.logoContainer}>
        {/* HIDDEN LOGIN TRIGGER - CLICK THE "AY" BADGE */}
        <TouchableOpacity 
            ...
            <GlassCard style={styles.heroCard}>
                ...
                <View style={[styles.actionRow, isMobile && { flexDirection: 'column' }]}>
                    ...
                    <TouchableOpacity 
                      ...
                      onPress={() => profile?.cv_url ? openLink(profile.cv_url) : Alert.alert('Info', 'CV is currently being updated.')}
                      activeOpacity={0.8}
                    >
                        <Download size={18} color={COLORS.text} style={{ marginRight: 8 }}/>
        ...
        <View style={[styles.heroImageContainer, isDesktop ? { width: '45%', alignItems: 'flex-end' } : { width: '100%', alignItems: 'center', marginBottom: SPACING.xl }]}>
            <GlassCard style={[styles.imageCard, { borderRadius: 1000 }]}>
                {profile?.profile_image_url ? (
                    ...
                    <View style={[styles.placeholderImage, { width: isMobile ? 240 : 400, height: isMobile ? 240 : 400, borderRadius: 1000 }]}>
                        <User size={80} color={COLORS.textDim}/>
                    </View>
                )}
            </GlassCard>
        </View>
        ...
        <View style={{ flex: isDesktop ? 6 : 1 }}>
            <GlassCard delay={200} style={styles.aboutCard}>
                ...
                <Text style={styles.bodyText}>
                    I am a dedicated <Text style={{color: COLORS.text, fontWeight: 'bold'}}>Java Fullstack Developer</Text> currently refining my craft at Lexicon. My passion stems from the ability to control the entire stack—from the pixel-perfect frontend to the secure database queries.
                    {'\n'}{'\n'}
                    I specialize in <Text style={{color: COLORS.primary}}>Java, Spring Boot, React</Text>, and cloud architecture. Beyond code, I am deeply invested in <Text style={{color: COLORS.secondary, fontWeight: 'bold'}}>cybersecurity and ethical hacking</Text>, ensuring that every application I build is robust against modern threats.
                </Text>
            </GlassCard>
        </View>
...
const styles = StyleSheet.create({
  ...
  primaryButton: { backgroundColor: COLORS.primary, paddingVertical: 14, paddingHorizontal: 28, borderRadius: 100, flexDirection: 'row', alignItems: 'center', gap: 8, justifyContent: 'center', shadowColor: COLORS.primary, shadowOpacity: 0.3, shadowRadius: 8 },
  primaryButtonText: { color: COLORS.background, fontWeight: 'bold', fontSize: 15 },
  secondaryButton: { flexDirection: 'row', alignItems: 'center', paddingVertical: 14, paddingHorizontal: 28, borderRadius: 100, borderWidth: 1, borderColor: COLORS.border, justifyContent: 'center' },
  ...
  certBadge: { flexDirection: 'row', alignItems: 'center', opacity: 0.7, marginTop: SPACING.s },
  certText: { color: COLORS.textDim, marginLeft: 6, fontSize: 11, fontWeight: '600' },
